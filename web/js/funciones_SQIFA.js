...
/** Capa de apoyo mediante JS al formulario SQIFA
 * @class FormSQIFA
...
/**
 * Define los procedimeintos estaticos del formulario SQIFA
 * @function
 **/
FormSQIFA.prototype.inicializar= function(){

  /**
   * @description El fomrulario es limpio al recargar la páguina
   **/
  $('#formulario_SQIFA')[0].reset();
  ...
  $('.botonradio').focusin(function() {
    ...
    nombre_bloque=nombre_radio.replace("radio_VorF_SQIFA_", "SQIFA_pregunta_");//se transforma en el nombre del bloque
    $("#"+nombre_bloque).addClass('SQIFA_bloque_pregunta_seleccionada');
    $(this).focusout(function() {
      var nombre_radio=$(this).attr('name');//nombreradio
      nombre_bloque=nombre_radio.replace("radio_VorF_SQIFA_", "SQIFA_pregunta_");//se transforma en el nombre del bloque
      $("#"+nombre_bloque).removeClass('SQIFA_bloque_pregunta_seleccionada');
    });
  });
   ...
   **/
      ...
      autoOpen: false,
          ...
          document.formulario_SQIFA.submit();
				},
				"Cancelar": function() {
					$(this).dialog("close");
				}
 ...
 * SQIFA. Las configuraciones por defecto de las validaciones (estilo grafico, etc) son heredadas
 ...
 **/
FormSQIFA.prototype.validar= function(){
    if(Main.getInstance().enableValidation === 'true'){
      jQuery.validator.addClassRules("botonradio", {
        required: true
      });
      return $("#formulario_SQIFA").valid(); //retorna true si valido, false si invalido
    } else{
...
$().ready(function() {
  var myFormularioSQIFA=new FormSQIFA();
  myFormularioSQIFA.inicializar();

   /** Si es válido muestro una confirSQIFAón sino no hago nada ya que el
    * método validar mostrará errores de validación y frenará el evento
    * submit.
    */
    $('form#formulario_SQIFA').submit(function(){
      if(myFormularioSQIFA.validar()){
