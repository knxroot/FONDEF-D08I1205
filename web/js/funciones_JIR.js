...
/** Capa de apoyo mediante JS al formulario JIR
 * @class FormJIR
...
/**
 * Define los procedimeintos estaticos del formulario JIR
 * @function
 **/
FormJIR.prototype.inicializar= function(){

  /**
   * @description El fomrulario es limpio al recargar la páguina
   **/
  $('#formulario_JIR')[0].reset();
  ...
  $('.botonradio').focusin(function() {
    ...
    nombre_bloque=nombre_radio.replace("radio_VorF_JIR_", "JIR_pregunta_");//se transforma en el nombre del bloque
    $("#"+nombre_bloque).addClass('JIR_bloque_pregunta_seleccionada');
    $(this).focusout(function() {
      var nombre_radio=$(this).attr('name');//nombreradio
      nombre_bloque=nombre_radio.replace("radio_VorF_JIR_", "JIR_pregunta_");//se transforma en el nombre del bloque
      $("#"+nombre_bloque).removeClass('JIR_bloque_pregunta_seleccionada');
    });
  });
   ...
   **/
      ...
      autoOpen: false,
          ...
          document.formulario_JIR.submit();
				},
				"Cancelar": function() {
					$(this).dialog("close");
				}
 ...
 * JIR. Las configuraciones por defecto de las validaciones (estilo grafico, etc) son heredadas
 ...
 **/
FormJIR.prototype.validar= function(){
    if(Main.getInstance().enableValidation === 'true'){
      jQuery.validator.addClassRules("botonradio", {
        required: true
      });
      return $("#formulario_JIR").valid(); //retorna true si valido, false si invalido
    } else{
...
$().ready(function() {
  var myFormularioJIR=new FormJIR();
  myFormularioJIR.inicializar();
    ...
    * submit.
    */
    $('form#formulario_JIR').submit(function(){
      if(myFormularioJIR.validar()){
