...
/** Capa de apoyo mediante JS al formulario CAIE
 * @class FormCAIE
...
/**
 * Define los procedimeintos estaticos del formulario CAIE
 * @function
 **/
FormCAIE.prototype.inicializar= function(){

  /**
   * @description El fomrulario es limpio al recargar la páguina
   **/
  $('#formulario_CAIE')[0].reset();
  ...
  $('.botonradio').focusin(function() {
    ...
    nombre_bloque=nombre_radio.replace("radio_VorF_CAIE_", "CAIE_pregunta_");//se transforma en el nombre del bloque
    $("#"+nombre_bloque).addClass('CAIE_bloque_pregunta_seleccionada');
    $(this).focusout(function() {
      var nombre_radio=$(this).attr('name');//nombreradio
      nombre_bloque=nombre_radio.replace("radio_VorF_CAIE_", "CAIE_pregunta_");//se transforma en el nombre del bloque
      $("#"+nombre_bloque).removeClass('CAIE_bloque_pregunta_seleccionada');
    });
  });
  ...
  $( "#dialogo-addcomentario").dialog({
    ...
    **/
   $('input[name="check_VorF_IRNC_52b"]').change(function () {
        if($(this).val()=="Otra"){
          $("#input_otra_iglesia").rules("add", {
           required: true
          });
          $("#input_otra_iglesia").show();
        }else{
          $("#input_otra_iglesia").rules("remove", "required");
          $("#input_otra_iglesia").hide();      
      }
   });
   ...
   **/
      ...
      autoOpen: false,
          ...
          document.formulario_CAIE.submit();
				},
				"Cancelar": function() {
					$(this).dialog("close");
				}
 ...
 * CAIE. Las configuraciones por defecto de las validaciones (estilo grafico, etc) son heredadas
 ...
 **/
FormCAIE.prototype.validar= function(){
    if(Main.getInstance().enableValidation === 'true'){
     jQuery.validator.addClassRules({
      ...
      return $("#formulario_CAIE").valid(); //retorna true si valido, false si invalido
    } else{
...
$().ready(function() {
  var myFormularioCAIE=new FormCAIE();
  myFormularioCAIE.inicializar();
    ...
    * submit.
    */
    $('form#formulario_CAIE').submit(function(){
      if(myFormularioCAIE.validar()){
