...
/** Capa de apoyo mediante JS al formulario MACI
 * @class FormMACI
...
/**
 * Define los procedimeintos estaticos del formulario MACI
 * @function
 **/
FormMACI.prototype.inicializar= function(){

  /**
   * @description El fomrulario es limpio al recargar la páguina
   **/
  $('#formulario_MACI')[0].reset();
  ...
  $('.botonradio').focusin(function() {
    ...
    nombre_bloque=nombre_radio.replace("radio_VorF_MACI_", "maci_pregunta_");//se transforma en el nombre del bloque
    $("#"+nombre_bloque).addClass('maci_bloque_pregunta_seleccionada');
    $(this).focusout(function() {
      var nombre_radio=$(this).attr('name');//nombreradio
      nombre_bloque=nombre_radio.replace("radio_VorF_MACI_", "maci_pregunta_");//se transforma en el nombre del bloque
      $("#"+nombre_bloque).removeClass('maci_bloque_pregunta_seleccionada');
    });
  });
   ...
   **/
      ...
      autoOpen: false,
          ...
          document.formulario_MACI.submit();
				},
				"Cancelar": function() {
					$(this).dialog("close");
				}
 ...
 * MACI. Las configuraciones por defecto de las validaciones (estilo grafico, etc) son heredadas
 ...
 **/
FormMACI.prototype.validar= function(){
    if(Main.getInstance().enableValidation === 'true'){
      jQuery.validator.addClassRules("botonradio", {
        required: true
      });
      return $("#formulario_MACI").valid(); //retorna true si valido, false si invalido
    } else{
...
$().ready(function() {
  var myFormularioMACI=new FormMACI();
  myFormularioMACI.inicializar();
    ...
    * submit.
    */
    $('form#formulario_MACI').submit(function(){
      if(myFormularioMACI.validar()){
