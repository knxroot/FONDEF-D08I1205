...
/** Capa de apoyo mediante JS al formulario CSVE
 * @class FormCSVE
...
/**
 * Define los procedimeintos estaticos del formulario CSVE
 * @function
 **/
FormCSVE.prototype.inicializar= function(){

  /**
   * @description El fomrulario es limpio al recargar la páguina
   **/
  $('#formulario_CSVE')[0].reset();
  ...
  $('.botonradio').focusin(function() {
    ...
    nombre_bloque=nombre_radio.replace("radio_VorF_CSVE_", "CSVE_pregunta_");//se transforma en el nombre del bloque
    $("#"+nombre_bloque).addClass('CSVE_bloque_pregunta_seleccionada');
    $(this).focusout(function() {
      var nombre_radio=$(this).attr('name');//nombreradio
      nombre_bloque=nombre_radio.replace("radio_VorF_CSVE_", "CSVE_pregunta_");//se transforma en el nombre del bloque
      $("#"+nombre_bloque).removeClass('CSVE_bloque_pregunta_seleccionada');
    });
  });
   ...
   **/
      ...
      autoOpen: false,
          ...
          document.formulario_CSVE.submit();
				},
				"Cancelar": function() {
					$(this).dialog("close");
				}
 ...
 * CSVE. Las configuraciones por defecto de las validaciones (estilo grafico, etc) son heredadas
 ...
 **/
FormCSVE.prototype.validar= function(){
    if(Main.getInstance().enableValidation === 'true'){
      jQuery.validator.addClassRules("botonradio", {
        required: true
      });
      return $("#formulario_CSVE").valid(); //retorna true si valido, false si invalido
    } else{
...
$().ready(function() {
  ...
  myFormularioCSVE.inicializar();
    ...
    * submit.
    */
    $('form#formulario_CSVE').submit(function(){
      if(myFormularioCSVE.validar()){
