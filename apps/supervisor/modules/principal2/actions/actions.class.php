<?php
class principal2Actions extends Actions
{
...
$SQL_CONSULTA_NOMBRE="SELECT `input_primer_nombre` , `input_otros_nombres` , `input_primer_apellido` , `input_segundo_apellido`
  ...
  */
  public function executeHome(sfWebRequest $request)
  {
    ...
    if($this->BD_Conectar()) {
    //WHERE `id_user_responsable_principal` = '{$this->getUser()->getGuardUser()->getId()}'
    $sql_listar_encuestados="SELECT `id` ,`first_name` ,`last_name`, `username` , `last_login` FROM `sf_guard_user` where `supervisor`= '{$this->getUser()->getGuardUser()->getId()}'";

    sfConfig::set('sf_escaping_strategy', false);
      $this->encuestadores = mysql_query($sql_listar_encuestados);
