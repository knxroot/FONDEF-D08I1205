...
class principalActions extends Actions
{
    ...
    public function executeDashboard(sfWebRequest $request)
    {
        ...
        $this->idEncuestador = $request->getParameter('idEncuestador');
        // redireccionar si no se ha indicado encuestado
        $this->forward404If(!$this->idEncuestado);

        $this->forward404If(!$this->idEncuestador);
        $idUser = $this->idEncuestador;

        
        //obtiene si las cusas estan cerradas o no
        
        $this->cuasasactuales = $this->esCerrado_appsupervisor($request, 'causasactuales_respuestas',$this->idEncuestador);
          $this->cuasasvigentes = $this->esCerrado_appsupervisor($request, 'causasvigentes_respuestas',$this->idEncuestador);
            $this->cuasasanteriores = $this->esCerrado_appsupervisor($request, 'causasanteriores_respuestas',$this->idEncuestador);
  ...
  $esCerrado=$this->esCerrado_appsupervisor($request, 'eda_respuestas',$this->idEncuestador);
    ...
    /* CAMBIO CONSENSO START */
        $esCerradoEged=$this->esCerrado2_appsupervisor($request, 'eged2_respuestas',0,$this->idEncuestador);
        $this->porcCompletadoEGED = $esCerradoEged*100;
    /* CAMBIO CONSENSO STOP */
        $esCerradodepado=$this->esCerrado2_appsupervisor($request, 'depado_respuestas',0,$this->idEncuestador);
        $this->porcCompletadodepado = $esCerradodepado*100;
        
         $esCerradoFerr=$this->esCerrado2_appsupervisor($request, 'ferr2_respuestas',0,$this->idEncuestador);
    ...
    /* CAMBIO CONSENSO START */
        $esCerradoIrnc=$this->esCerrado2_appsupervisor($request, 'irnc2_respuestas',0,$this->idEncuestador);
        $this->porcCompletadoIRNC = $esCerradoIrnc*100;
    /* CAMBIO CONSENSO STOP */
        
        $esCerradoFcmf=$this->esCerrado2_appsupervisor($request, 'fcmf_respuestas',0,$this->idEncuestador);
...
$SQL_CONSULTA_NOMBRE="SELECT `input_primer_nombre` , `input_otros_nombres` , `input_primer_apellido` , `input_segundo_apellido`
  ...
  $this->nombreEncuestado=$r[0]." ".$r[1]." ".$r[2]." ".$r[3];
    ...
    if($cierreseged>1){
      ...
      $esCerradoEged=$this->esCerrado2_appsupervisor($request, 'eged2_respuestas',1,$this->idEncuestador);
    ...
    if($cierresferr>1){
      ...
      $esCerradoFerr=$this->esCerrado2_appsupervisor($request, 'ferr2_respuestas',1,$this->idEncuestador);
    ...
    if($cierresirnc>1){
      ...
      $esCerradoIrnc=$this->esCerrado2_appsupervisor($request, 'irnc2_respuestas',1,$this->idEncuestador);
    ...
    if($cierresfcmf>1){
      ...
      $esCerradoFcmf=$this->esCerrado2_appsupervisor($request, 'fcmf_respuestas',1,$this->idEncuestador);
  ...
  */
  public function executeHome(sfWebRequest $request)
  {
    ...
    $sql_listar_encuestados="SELECT `id_encuestado` ,`input_run_nconverificador`, `input_primer_nombre` , `input_otros_nombres` , `input_primer_apellido` , `input_segundo_apellido` , `input_fnacimiento` , `created_at` , `updated_at` FROM `encuestado` WHERE `id_user_responsable_principal` = '{$this->idEncuestador}'";
    sfConfig::set('sf_escaping_strategy', false);
      $this->encuestados = mysql_query($sql_listar_encuestados);

 $sql_listar_coevaluados="SELECT `id_encuestado` ,`input_run_nconverificador`, `input_primer_nombre` , `input_otros_nombres` , `input_primer_apellido` , `input_segundo_apellido` , `input_fnacimiento` , `created_at` , `updated_at` FROM `encuestado` WHERE `select_user_responsable_secundario1` = '{$this->idEncuestador}'";
    sfConfig::set('sf_escaping_strategy', false);
