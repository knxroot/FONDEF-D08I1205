...
<form action="<?php echo url_for('CausasActuales/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<?php endif; ?>
  <table>
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          &nbsp;<a href="<?php echo url_for('CausasActuales/index') ?>">Back to list</a>
          <?php if (!$form->getObject()->isNew()): ?>
            &nbsp;<?php echo link_to('Delete', 'CausasActuales/delete?id='.$form->getObject()->getId(), array('method' => 'delete', 'confirm' => 'Are you sure?')) ?>
          <?php endif; ?>
    ...
    <tbody>
      ...
      <tr>
        <th><?php echo $form['input_ruk3']->renderLabel() ?></th>
        <td>
          <?php echo $form['input_ruk3']->renderError() ?>
          <?php echo $form['input_ruk3'] ?>
        </td>
      </tr>
    </tbody>
  </table>
