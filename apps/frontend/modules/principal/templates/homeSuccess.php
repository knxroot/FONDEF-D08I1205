...
<table cellspacing="1" cellpadding="0" border="0" class="tablesorter" id="tablesorter-listaEncuestados">
		<thead>
			<tr>
				...
				<th class="header">Tiempo total empleado</th>
      ...
      if($i%2){echo "<tr class='even'>";}else{echo "<tr class='odd'>";}
      	echo "<td>{$encuestado['input_run_nconverificador']}</td>";
				...
				echo "<td>{$encuestado['created_at']}</td>";
				 $sql_listar_tiempo="SELECT TIME_FORMAT(SUM(tiempo), '%H:%i:%s') AS sumahoras FROM `tiempo` WHERE `id_user` = '{$id_usuario}' AND `id_encuestado` = '{$encuestado['id_encuestado']}'";
      sfConfig::set('sf_escaping_strategy', false);
      $tiempos = mysql_query($sql_listar_tiempo);
      
      while($tiempo=mysql_fetch_array($tiempos)){          
          
          echo "<td>{$tiempo['sumahoras']}</td>";
      }
        $url="principal/dashboard?idEncuestado={$encuestado['id_encuestado']}";
        echo "<td class='operaciones'><a href='"; echo url_for($url); echo "'>EDITAR</a></td>";
			echo "</tr>";
      $i++;
      }
    ?>
    ...
    <?php
      ...
      if($i%2){echo "<tr class='even'>";}else{echo "<tr class='odd'>";}
      	echo "<td>{$coevaluado['input_run_nconverificador']}</td>";
				...
				echo "<td>{$coevaluado['created_at']}</td>";
                                ...
                                $sql_listar_tiempo="SELECT TIME_FORMAT(SUM(tiempo), '%H:%i:%s') AS sumahoras FROM `tiempo` WHERE `id_user` = '{$id_usuario}' AND `id_encuestado` = '{$coevaluado['id_encuestado']}'";
      sfConfig::set('sf_escaping_strategy', false);
      $tiempos = mysql_query($sql_listar_tiempo);
      ...
      while($tiempo=mysql_fetch_array($tiempos)){          
        ...
        $url="principal/dashboard?idEncuestado={$coevaluado['id_encuestado']}";
        echo "<td class='operaciones'><a href='"; echo url_for($url); echo "'>EDITAR</a></td>";
			echo "</tr>";
      $i++;
      }
