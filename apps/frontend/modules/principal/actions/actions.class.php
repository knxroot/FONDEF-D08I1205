...
$SQL_CONSULTA_NOMBRE="SELECT `input_primer_nombre` , `input_otros_nombres` , `input_primer_apellido` , `input_segundo_apellido`
  ...
  */
  public function executeHome(sfWebRequest $request)
  {
    ...
    if($this->BD_Conectar()) {
$this->id_usuario=$this->getUser()->getGuardUser()->getId();
    ...
    public function executeProcesarForm(sfWebRequest $request)
    {
        ...
        $this->pdo->beginTransaction(); 
         ...
         mysql_query($actualizador);
          $query = $this->pdo->prepare($actualizador);
...
try {
    $dbh = new PDO("mysql:host=$hostname;dbname=psico", $username, $password);
    /*** echo a message saying we have connected ***/


    /*** INSERT data ***/
     $count = $dbh->exec("UPDATE `psico`.`encuestado` SET `created_at` = NOW( ) WHERE `encuestado`.`id_encuestado` ={$id}");
