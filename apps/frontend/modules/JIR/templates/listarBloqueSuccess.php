<?php slot('title') ?>
  <?php  echo "Formulario a JIR ({$porcCompletado} % completado)";?>
<?php end_slot(); ?>

<?php $url_next_post=url_for('JIR/proximoBloque?idEncuestado='.$idEncuestado);?>
  ...
  <div id="dialogo-instrucciones" style="display: none;" title="Instrucciones JIR">
	<p>
     ...
     A continuación te presentamos una serie de frases que los jóvenes usan para describirse a sí mismos. Se presentan aquí para ayudarte a describir tus sentimientos y actitudes. 
      ...
      <br><br>
      Cuando decidas que la frase <b>describe tu situación personal</b>, o <b>estás de acuerdo</b> selecciona la letra <b>V</b> para indicar que es <b>V</b>erdadera.
      <br>
      Si decides que la frase <b>no describe tu situación personal</b>, o <b>estás en desacuerdo</b> selecciona la letra <b>F</b> para indicar que es <b>F</b>alsa. <br>
      <br>
      <?php echo image_tag('ejemplorespuesta.png', 'alt=Selecciona el radio que tenga el label V o el label F') ?>
	</p>
	
	<p>Comienza leyendo cada frase, una a una y trata de responder a todas, aun cuando no estés muy seguro de tu decisión. Si lo has intentado pero no puedes decidirte, entonces responde la frase escribiendo la letra <b>F (Falso).</b></p>
  
  <ul>
    <li>Si te equivocas o deseas cambiar alguna respuesta, puedes hacerlo cambiando la opción seleccionada.</li>
    <li>Si tienes dudas o deseas que te expliquen una palabra o frase, <b>pide ayuda.</b></li>
    <li>Procura responder <b>todas las frases</b></li>
    <li><b>Trabaja lo más rápido posible</b>, según tu ritmo. No hay tiempo límite.</li>
 </ul>
 <br>
...
<form id="formulario_JIR" name="formulario_JIR" class="form" method="post" action="<?php echo $url_next_post; ?>">
  <?php if(!($es_ultimo_bloque)): ?>
  <div class="grid-12-12 ui-widget-header ui-corner-all" style="position: relative;">Formulario JIR - <?php echo $porcCompletado; ?> % completado
  <span class="icono-ver-instrucciones" onclick="$('#dialogo-instrucciones').dialog('open');"></span>
  </div>
<?php foreach ($Preguntas_JIR as $pregunta):?>
  <!-- Comienzo Bloque Pregunta -->
  <div id="JIR_pregunta_<?php echo $pregunta["id_pregunta"];?>" class="JIR_bloque_pregunta">
		<div class="grid-8-12">
    ...
    <div class="grid-3-12">
      <ul class="form-list-rdo">
        <li><input class="botonradio" name="radio_VorF_JIR_<?php echo $pregunta["id_pregunta"];?>" type="radio" value="V" /><label class="form-lbl">V</label></li>
        <li><input class="botonradio" name="radio_VorF_JIR_<?php echo $pregunta["id_pregunta"];?>" type="radio" value="F" /><label class="form-lbl">F</label></li>
			</ul>
    </div>
...
<script type="text/javascript">
  ...
  if(is_show_instruccions==null){ var is_show_instruccions=false;$("#formulario_JIR").hide();alert("Al parecer ha habido un problema de connección, es recomendable que recarge la página");}
