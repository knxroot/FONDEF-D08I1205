<?php slot('title') ?>
  <?php  echo "Formulario a MACI ({$porcCompletado} % completado)";?>
<?php end_slot(); ?>

<?php $url_next_post=url_for('MACI/proximoBloque?idEncuestado='.$idEncuestado);?>

<!-- Comienzo Bloque Dialogo Instrucciones -->
  <div id="dialogo-instrucciones" style="display: none;" title="Instrucciones MACI">
	<p>
...
<form id="formulario_MACI" name="formulario_MACI" class="form" method="post" action="<?php echo $url_next_post; ?>">
  <?php if(!($es_ultimo_bloque)): ?>
  <div class="grid-12-12 ui-widget-header ui-corner-all" style="position: relative;">Formulario MACI - <?php echo $porcCompletado; ?> % completado
  <span class="icono-ver-instrucciones" onclick="$('#dialogo-instrucciones').dialog('open');"></span>
  </div>
<?php foreach ($Preguntas_MACI as $pregunta):?>
  <!-- Comienzo Bloque Pregunta -->
  <div id="maci_pregunta_<?php echo $pregunta["id_pregunta"];?>" class="maci_bloque_pregunta">
		<div class="grid-8-12">
    ...
    <div class="grid-3-12">
      <ul class="form-list-rdo">
        <li><input class="botonradio" name="radio_VorF_MACI_<?php echo $pregunta["id_pregunta"];?>" type="radio" value="V" /><label class="form-lbl">V</label></li>
        <li><input class="botonradio" name="radio_VorF_MACI_<?php echo $pregunta["id_pregunta"];?>" type="radio" value="F" /><label class="form-lbl">F</label></li>
			</ul>
    </div>
...
<script type="text/javascript">
  ...
  if(is_show_instruccions==null){ var is_show_instruccions=false;$("#formulario_MACI").hide();alert("Al parecer ha habido un problema de connección, es recomendable que recarge la página");}

 if(is_show_instruccions){
    $("#formulario_MACI").hide();
  }else{}
